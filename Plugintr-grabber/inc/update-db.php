...
function grabberdbupdate_function()
{
    ...
    if (defined('TR_GRABBER_TIMELIMIT') && TR_GRABBER_TIMELIMIT) {
        set_time_limit(TR_GRABBER_TIMELIMIT);
    }
    if (defined('TR_GRABBER_MEMOYLIMIT') && TR_GRABBER_MEMOYLIMIT) {
        ini_set('memory_limit', TR_GRABBER_MEMOYLIMIT);
    }
                ...
                while ($query_movies->have_posts()) {
                    ...
                    if ($trmovieslinks != '') {
                        foreach ($trmovieslinks as &$value) {
                            ...
                            if (isset($array_links[$i]['link']) and !empty($array_links[$i]['link'])) {
                                $count .= $i . ',';
                                update_post_meta(get_the_ID(), 'trglinks_' . $i, serialize($array_links[$i]));
                            }

                            $i++;
                        }

                        if (isset($count) and !empty($count)) update_post_meta(get_the_ID(), 'trgrabber_tlinks', count($array_links));
                        delete_post_meta(get_the_ID(), 'trmovieslink');
                    }
                ...
                if ($trmovieslinks != '') {
                    ...
                    if (isset($array_links)) {
                        ...
                        foreach ($array_links as &$value) {
                            ...
                            if (isset($array_links[$i]['link']) and !empty($array_links[$i]['link'])) {
                                $count = $i;
                                update_term_meta($s->term_id, 'trglinks_' . $i, serialize($array_links[$i]));
                            }
                        ...
                        if (isset($count) and !empty($count)) update_term_meta($s->term_id, 'trgrabber_tlinks', $count + 1);
                        delete_term_meta($s->term_id, 'trmovieslink');
                    }
                }
    ...
    <style>
        ...
        .trgrabber_loading span.grabberporc {
            position: absolute;
            left: 0;
            right: 0;
            top: 5px;
