...
function trgrabber_template( $template ) {
    if( get_query_var('AnimeEmbed') != '' and get_query_var('location') != '' ) {
...
function trgrabber_template_redirect() {
    if( get_query_var('trdownload') != '' and get_query_var('location') != '' ) {
        
        if(isset($_GET['t'] )){
            if($_GET['t'] == 'ser'){
                $type = get_term_meta( intval(get_query_var('location')), 'tr_id_post', true );
            } 
        
            $link = $type == '' ? unserialize ( get_post_meta( intval(get_query_var('location')), 'trglinks_'.intval(get_query_var('trdownload')), true ) ) : unserialize ( get_term_meta( intval(get_query_var('location')), 'trglinks_'.intval(get_query_var('trdownload')), true ) );
            $link = base64_decode( $link['link'] );
            wp_redirect( $link );
            die;
        } else {
            $link = unserialize ( get_post_meta( intval(get_query_var('location')), 'trglinks_'.intval(get_query_var('trdownload')), true ) );
            $link = base64_decode( $link['link'] );
            wp_redirect( $link );
            die;
        }
